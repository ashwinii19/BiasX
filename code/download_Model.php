...
$sql = "SELECT received_file4_data FROM received_files WHERE id = ?";
$stmt = $conn->prepare($sql);
$stmt->bind_param("i", $fileId);
$stmt->execute();
$stmt->store_result();
$stmt->bind_result($received_file4_data);
$stmt->fetch();

if (!$received_file4_data) {
    die('File not found.');
}

// Prepare the CSV file
header('Content-Type: text/pkl');
header('Content-Disposition: attachment; filename="downloaded_file.pkl"');
...
$lines = preg_split('/\s*;\s*/', trim($received_file4_data)); // Split by `;`
