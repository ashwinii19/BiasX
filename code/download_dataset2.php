...
$sql = "SELECT received_file3_data FROM received_files WHERE id = ?";
$stmt = $conn->prepare($sql);
$stmt->bind_param("i", $fileId);
$stmt->execute();
$stmt->store_result();
$stmt->bind_result($received_file3_data);
$stmt->fetch();

if (!$received_file3_data) {
    die('File not found.');
}
...
$lines = preg_split('/\r\n|\r|\n/', trim($received_file3_data)); // Split by `;`
